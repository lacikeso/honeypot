...
void print_udp_packet(const u_char *Buffer , int Size)
{
	...
	snprintf (s_port, 9, "%d",ntohs(udph->source));

	/* cielovy port */
	snprintf (d_port, 9, "%d",ntohs(udph->dest));
...
void PrintData (const u_char *data , int Size, char *ipecka, char *ipecka_d, char *source_port, char *destination_port)
{
        ...
        //char *floods = strtok(data, "\n");
        snprintf (buff1, sizeof(buff1), "%s",data);

        //INVITE FLOOD
         if (strstr(buff1, "INVITE") != NULL)
         {
                printf("%s\n", buff1);
                /* ak to bol prvy INVITE spusti casovac, uloz do premennej hodnotu IP adresy odosielatela INVITE spravy */
                if (inv == 0)
                {
                        snprintf (pch, sizeof(pch), "%s",ipecka);
                        printf("%s %f\n",ipecka, gettimeofday(&tv1, NULL));
                }
                if(strstr(pch, ipecka) != NULL)
                {
                        printf("%d\n", inv);
                        ++inv;
                        ...
                        if (inv == atoi(configstruct.packet_num))
                        {
                                ...
                                if(vysledok < atof(configstruct.flood_interval))
                                {
                                        ...
                                        if(strcmp(configstruct.sipdomain, ipecka) == 0){
                                                ParseFloodPacket(data,"INVITE", ipecka_d, ipecka, destination_port, source_port);
                                        }
                                        else{
        ...
        /***********************************************************************/
        if(strstr(buff1, "REGISTER") != NULL)
        {
                if (reg == 0)
                {
                                strncpy(pch2, ipecka, sizeof(pch2));
                ...
                if(strcmp(pch2, ipecka) == 0)
                {
                        ...
                        if (reg == atoi(configstruct.packet_num))
                        {
                                ...
                                {
                                        if(strcmp(configstruct.sipdomain, ipecka) == 0){
                                                ParseFloodPacket(data,"REGISTER", ipecka_d, ipecka, destination_port, source_port);
                                        }
                                        else{
        ...
        /**********************************************************************/
        if(strstr(buff1, "OPTIONS") != NULL)
        {
                if (opt == 0)
                {
                        strncpy(pch3, ipecka, sizeof(pch3));
                        gettimeofday(&tv1, NULL);
                }
                printf("OPTIONS\n");
                ...
                if(strcmp(pch3, ipecka) == 0)
                {
                        ...
                        if (opt == atoi(configstruct.packet_num))
                        {
                                ...
                                if(vysledok < atof(configstruct.flood_interval))
                                {
                                        ...
                                        if(strcmp(configstruct.sipdomain, ipecka) == 0){
                                                ParseFloodPacket(data,"OPTIONS", ipecka_d, ipecka, destination_port, source_port);
                                        }
                                        else{
        ...
        if(curl)
        {
                ...
                for (i = 0; i < cJSON_GetArraySize(root); i++)
                {
                        ...
                        if(i == 2)
                        {
                                ...
                                cJSON* subitem2 = cJSON_GetArrayItem(subitem, 0);
                                 cJSON* subitem3 = cJSON_GetArrayItem(subitem2, 0);
                                cJSON* subitem4 = cJSON_GetArrayItem(subitem3, 4);
                                cJSON* subitem5 = cJSON_GetArrayItem(subitem4, 0);

                                for (k= 0 ; k< cJSON_GetArraySize(subitem5) ; k++)
                                {

                                        cJSON * subitem = cJSON_GetArrayItem(subitem5, k);
                                        char *name1 = cJSON_GetObjectItem(subitem, "name") -> valuestring;
                                        if (strcmp(name1, "country") == 0)
                                        {
                                ...
                                for (j = 0 ; j < cJSON_GetArraySize(item4) ; j++)
                                {
                                        ...
                                        char *name2 = cJSON_GetObjectItem(subitem, "name") -> valuestring;
                                        if (strcmp(name2, "address") == 0)
                                        {
                                                        char *index2 = cJSON_GetObjectItem(subitem, "value") -> valuestring;
                                                        strcat(buff, ";");
                                                        strcat(buff, index2);
...
void *Server()
{
	...
	bzero(&address, sizeof(address));
        ...
        inet_pton(AF_INET, configstruct.server_ip, &address.sin_addr);
...
//      printf("IPECKA :  %s\n", data);
        ...
        snprintf(buff1, 1020, "%s FLOOOD;%s;%s;%s;%s;%s\n", flood, pole, ip_dest, sport, dport, curling(pole) );
        printf("%s\n", buff1);
        SendToHoneypot(buff1);
